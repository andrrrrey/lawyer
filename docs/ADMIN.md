...
## Ночной пересчёт и журнал ошибок

Worker автоматически запускает полный пересчёт один раз в сутки в 03:00 по
московскому времени. Днём выполняется только короткая сверка изменившихся CRM-
записей каждые 5 минут; тяжёлая часовая загрузка отключена.

Журнал worker сохраняется на сервере в `/opt/lawyer/logs/worker.log`. Файл
ротируется при достижении 10 МБ, хранится до 10 предыдущих файлов. В журнал
попадают запуск и итог ночной операции, ошибки отдельных источников и полный
...
## Ручные расходы
В разделе «Настройки → Расходы» можно заносить прочие расходы по статьям. Для
рекламных расходов, которые должны влиять на ROMI, включите флаг «Учитывать в ROMI» и
выберите канал. Статьи без этого флага видны в блоке «Расходы по статьям», но не попадают в
формулу ROMI. Автоматически загружаемые расходы Яндекс Директа повторно вручную не вносятся.
